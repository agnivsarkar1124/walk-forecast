...
function initFromState() {
  if (state.home) {
    ...
    homeLatEl.value = state.home.lat ?? '';
    homeLonEl.value = state.home.lon ?? '';
  }
  renderClassList();
  renderStopList();
}
  ...
  state.classes.forEach((c, idx) => {
    ...
    li.innerHTML = `
      <div>
        ...
        <div class="meta">${c.lat.toFixed(4)}, ${c.lon.toFixed(4)} · ${c.start}–${c.end} · ${days}</div>
      </div>
...
function startClassEdit(idx) {
  ...
  document.getElementById('c-lon').value = c.lon;
  ...
  state.stops.forEach((s, idx) => {
    ...
    li.innerHTML = `
      <div>
        ...
        <div class="meta">${s.lat.toFixed(4)}, ${s.lon.toFixed(4)} · ${s.start}–${s.end} · ${days}</div>
      </div>
      <div class="item-actions">
        <button class="edit-btn" data-idx="${idx}" title="Edit">✎</button>
        <button class="remove-btn" data-idx="${idx}" title="Remove">✕</button>
...
// Attempts to set home, gated behind the university-proximity check. Only
// writes to state/localStorage if the check passes. Returns true/false (or
// throws on a genuine network failure, which callers should treat distinctly
// from "not near a university" so a flaky connection isn't mistaken for a
// rejected location).
async function trySetHome(lat, lon, label) {
  setStatus('Checking that this is near a university campus…', null);
  const nearUni = await checkNearUniversity(lat, lon);
  if (!nearUni) {
    setStatus('⚠ Home must be within ~2 miles of a university campus — location not saved.', 'error');
    return false;
  }
  state.home = { label: label || 'Home', lat, lon, verified: true };
  saveState();
  invalidateClimatologyCache();
...
[homeLabelEl, homeLatEl, homeLonEl].forEach(el => {
  el.addEventListener('change', async () => {
    const lat = parseFloat(homeLatEl.value);
    const lon = parseFloat(homeLonEl.value);
    if (isNaN(lat) || isNaN(lon)) return;
    homeVerificationInFlight = trySetHome(lat, lon, homeLabelEl.value)
      .catch(err => {
        console.error('University proximity check failed:', err);
...
classForm.addEventListener('submit', (e) => {
  ...
  if (!name || isNaN(lat) || isNaN(lon) || !start || !end || days.length === 0) {
  ...
  if (editingClassIndex !== null) {
    state.classes[editingClassIndex] = { name, lat, lon, start, end, days };
    setStatus('Class updated.', 'ok');
    cancelClassEdit();
  } else {
    state.classes.push({ name, lat, lon, start, end, days });
    classForm.reset();
...
stopForm.addEventListener('submit', (e) => {
  ...
  if (editingStopIndex !== null) {
    state.stops[editingStopIndex] = { name, lat, lon, start, end, days };
    setStatus('Stop updated.', 'ok');
    cancelStopEdit();
  } else {
    state.stops.push({ name, lat, lon, start, end, days });
    stopForm.reset();
...
async function ensureHomeSet() {
  if (homeVerificationInFlight) await homeVerificationInFlight; // let any pending check from editing the fields finish first

  const inputLat = parseFloat(homeLatEl.value);
  const inputLon = parseFloat(homeLonEl.value);
  const inputLabel = homeLabelEl.value;
  const fieldsAreValid = !isNaN(inputLat) && !isNaN(inputLon);

  // Has anything changed since the last time we verified+saved a home? If the
  // fields still match what's saved, there's no need to re-verify or refetch.
  const inputMatchesSavedHome = state.home && fieldsAreValid &&
    Math.abs(inputLat - state.home.lat) < 1e-6 &&
    Math.abs(inputLon - state.home.lon) < 1e-6;

  if (inputMatchesSavedHome && state.home.verified) {
    return true;
  }

  // Fields have something in them that doesn't match the saved home (or there's
  // no saved home at all) — always trust what's CURRENTLY in the fields over
  // whatever was saved before, so editing home and immediately hitting Generate
  // can never silently fall back to an old location.
  if (fieldsAreValid) {
    try {
      return await trySetHome(inputLat, inputLon, inputLabel);
    } catch (err) {
      console.error('University proximity check failed:', err);
      setStatus('Could not verify university proximity (network issue) — try again. (' + err.message + ')', 'error');
      return false;
  ...
  // Fields are empty/invalid right now, but a previously verified home exists — use it.
  if (state.home && !isNaN(state.home.lat) && !isNaN(state.home.lon)) {
    if (state.home.verified) return true;
    try {
      const ok = await trySetHome(state.home.lat, state.home.lon, state.home.label);
